...
class MetaPruner:
    ...
    def __init__(
        ...
        print("metapruner.py MetaPruner init call build_dependency!!!!")#jliu
        ...
        print("metapruner.py MetaPruner ignored_layers!!!!")
        self.ignored_layers = []
        if ignored_layers:
        ...
        print("metapruner.py Record initial status") #jliu
        ...
        print("metapruner.py detect group_convs_&_group_norms")#jliu
    ...
    def step(self, interactive=False):
        self.current_step += 1
        print("metpapruner.py MetaPruner pruner.step self.global_pruning, interactive",self.global_pruning, interactive)#jliu False False
        #pdb.set_trace()
    ...
    def prune_local(self):
        if self.current_step > self.iterative_steps:
            return
        print("@@@@@@@metpapruner.py prune_local") #jliu
        for group in self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types, root_instances=self.root_instances):
            # check pruning rate
            if self._check_sparsity(group):
                ...
                ch_groups = self.get_channel_groups(group)
                consecutive_groups = self.get_consecutive_groups(group)
                print("metpapruner.py prune_local call ->estimate_importance") #jliu
