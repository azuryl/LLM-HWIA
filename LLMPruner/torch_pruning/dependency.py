...
class Group(object):
    ...
    def prune(self, idxs=None, record_history=True, **kwargs):
        ...
        """
        print("dependency.py class Group.prune@@@@@@@") #jliu
        if idxs is not None:
...
class DependencyGraph(object):
    ...
    def build_dependency(
        ...
        print("dependency.py build_dependency!!!!!!!!!!!!")#jliu
        ...
        print("dependency.py Detect unwrapped nn.parameters!!!!!!!!!!!!") #jliu                  
        wrapped_parameters = []
        ...
        print("dependency.py Build computational_graph_by_tracing->self._trace") #jliu
        self.module2node = self._trace(
            model, example_inputs, forward_fn, output_transform=output_transform
        )

        print("dependency.py call Build_dependency_graph......")#jliu
    ...
    def _trace(self, model, example_inputs, forward_fn, output_transform):
        ...
        print("dependency.py _trace Feed forward_and_record_gradient functions of prunable modules") #jliu
        if forward_fn is not None:
            #print("dependency.py _trace Feed forward_and_record_gradient functions of prunable modules") #jliu
            out = forward_fn(model, example_inputs)
        elif isinstance(example_inputs, dict):
            #print("dependency.py _trace isinstance(example_inputs->model(**example_inputs)")#jliu
            out = model(**example_inputs)
        else:
            try:
                print("dependency.py _trace Feed forward_and_record_gradient_else_try>>)")#jliu
                out = model(*example_inputs)
                print("dependency.py _trace Feed forward_and_record_gradient_else_try_out):",out)#jliu
            except:
                print("dependency.py _trace Feed forward_and_record_gradient_else_except)")#jliu
                out = model(example_inputs)
                print("dependency.py _trace Feed forward_and_record_gradient_else_except_out)")#jliu
