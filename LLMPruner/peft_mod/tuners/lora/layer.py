...
class LoraLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, **kwargs) -> None:
        ...
        #print("#######LoraLayer self.lora_A",self.lora_A)
        # For Embedding layer
        ...
        self.kwargs = kwargs
        self.use_rslora: dict[str, bool] = {} #jliu
    ...
    def update_layer(
        ...
        print("####update_layer use_rslora,use_dora:",use_rslora,use_dora) #jliu
        self.lora_A[adapter_name] = nn.Linear(self.in_features, r, bias=False)
        self.lora_B[adapter_name] = nn.Linear(r, self.out_features, bias=False)
        if use_rslora :
            self.scaling[adapter_name] = lora_alpha / math.sqrt(r)
            self.use_rslora[adapter_name] = True  #jliu
            print("t##########update_layer  scaling,lora_alpha,r,use rslora",self.scaling[adapter_name],lora_alpha,r,self.use_rslora[adapter_name],)
        ...
        #self.use_dora[adapter_name] = False  #jliu
        if use_dora:
            print("!!!!!LoraLayer update_layer use_dora,use_rslora,lora_alpha,r",use_dora,use_rslora,lora_alpha,r)
    ...
    def dora_init(self, adapter_name: str) -> None:
        ...
        #print("dora_init########## scaling,use rslora",self.scaling[adapter_name],self.use_rslora[adapter_name])
        
        scaling = self.scaling[adapter_name]
        if self.use_rslora:
            dora_scaling = self.lora_alpha[adapter_name] / self.r[adapter_name] #jliu
            print("dora_init########## scaling,use rslora",self.scaling[adapter_name],self.use_rslora[adapter_name],dora_scaling)
            print("\n")   
        with gather_params_ctx(self.get_base_layer()):
            ...
            if weight.data.ndim == 4:  # For handling LoRAs applied to Conv2Ds.
                ...
                #print("VVVVVlayer.py lora_A.weight.dtype",lora_A.weight.dtype) 
                #print("^^^^^^layer.py lora_A.weight.dtype",lora_A.weight.dtype)
                lora_weight = lora_B.weight.float() @ lora_A.weight.float() #jliu
                '''
                device = lora_B.weight.device#jliu
            ...
            if self.use_rslora:
                scaling = dora_scaling #jliu
            weight_norm = self._get_weight_norm(weight, lora_weight, scaling)#jliu
    ...
    def _apply_dora(self, x, lora_A, lora_B, scaling, active_adapter):
        ...
        """
        if self.use_rslora:
            dora_scaling = self.lora_alpha[active_adapter] / self.r[active_adapter] #jliu
        else:
            dora_scaling = scaling
        ...
        weight_norm = self._get_weight_norm(weight, lora_weight, dora_scaling) #jliu
...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #use_dora = True
        print("l#####ayer.py Linear init use_dora :",use_dora)  #jliu
        print("l#####ayer.py Linear inituse_rslora :",use_rslora)  #jliu
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        ...
        print("@@@@@@@###layer.py Linear merge:",use_dora)
    ...
    def unmerge(self) -> None:
        ...
        """
        print("@@@@@###layer.py Linear unmerge:",use_dora)
        if not self.merged:
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                #print("\n VVVVVV layer.py,use_dora:",self.use_dora[active_adapter]) #jliu
                if not self.use_dora[active_adapter]:
                    result = result + lora_B(lora_A(dropout(x))) * scaling
                else:
                    
                    #print("##forward scaling:",scaling)
                    x = dropout(x)
...
class Embedding(nn.Module, LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, use_dora):
        ...
        print("\n Embedding  update_layeVVVVVVVVVVVV") #jliu
        self.r[adapter_name] = r
        self.lora_alpha[adapter_name] = lora_alpha
        if lora_dropout > 0.0:
            lora_dropout_layer = nn.Dropout(p=lora_dropout)
