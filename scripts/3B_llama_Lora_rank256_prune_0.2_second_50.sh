...
tune_ckpt_path='llama_3B_0.2_param_second_block_wise_N50_Lora_r256'
num_examples='50'
...
#CUDA_VISIBLE_DEVICES=$nGPU python post_training.py --prune_model prune_log/$prune_ckpt_path/pytorch_model.bin --data_path yahma/alpaca-cleaned --output_dir tune_log/$tune_ckpt_path --wandb_project llama_tune --lora_r $lora_r --num_epochs 2 --learning_rate 1e-4 --batch_size 64
CUDA_VISIBLE_DEVICES=$nGPU python fine-tuning.py --prune_model prune_log/$prune_ckpt_path/pytorch_model.bin --data_path yahma/alpaca-cleaned --output_dir tune_log/$tune_ckpt_path --wandb_project llama_tune --lora_r $lora_r --num_epochs 2 --learning_rate 1e-4 --batch_size 64   --use_dora False --use_rslora False
